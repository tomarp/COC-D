...
# Personalized Models for Car-Comfort 
...
### Data (Physiological & Meteorological):
    1. (PSY) Psychological (Empatica)
        - (PHY-TEMP) Temperature Sensor (degree Cel.)
        - (PHY-EDA) Electrodermal activity sensor (micro sec.)
        - (PHY-BVP) Photoplethysmograph (BVP)
        - (PHY-ACC) Accelerometer 3-axis sensor (ACC)
        - (PHY-IBI) Time of each heart beat and the duration between consecutive heart beats
        - (PHY-HR) Average heart rate extracted from the BVP signal, heart beat (BVP)
        - (PHY-TAGS) Event mark times
    
    2. (MET) Meteorological Sensors
        - (MET-N1) 
        - (MET-N2)
        - (MET-N3)
        - (MET-N4)
...
**Empatica:** ~E4 wrist-worn wearable device
